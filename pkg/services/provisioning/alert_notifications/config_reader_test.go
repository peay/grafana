...
var (
	...
	unknownVersion            = "./test-configs/unknown-version"
...
func TestNotificationAsConfig(t *testing.T) {
	Convey("Testing notification as configuration", t, func() {
		...
		Convey("unknown version should return error", func() {
			cfgProvifer := &configReader{log: log.New("test logger")}
			_, err := cfgProvifer.readConfig(unknownVersion)
			So(err, ShouldEqual, ErrInvalidApiVersion)
		})
		...
		Convey("can read all properties from version 0", func() {
			...
			So(ntCfg.ApiVersion, ShouldEqual, 0)
